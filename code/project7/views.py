...
from user.models import UserFileModel
import os
...
from user.models import UserAppCreatModel
...
def resturl(request,id):
    ...
    if request.method == 'GET':
        if role=='user':
            ...
            return render(request,'user/editfilesdata.html',dict)
        elif role=='admin':
...
def uploadfile(request):
    ...
    if role =='user':
        ...
        return  render(request,'user/uploadfile.html',{'objects':dict})
    elif role =='admin':
        return Response(status = status.HTTP_405_METHOD_NOT_ALLOWED)
